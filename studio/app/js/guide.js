...
export const GUIDE = [
  ...
  },
  {
    ...
    body: [
      ...
      ['Making one', 'A name, a shape (vertical, landscape, square, portrait, cinemascope), a size for that shape from light to 4K or a custom one that keeps the shape, a frame rate, and the colour behind the picture. All of these are awkward to change once there are clips on the timeline, which is why they are asked here. The line under the controls says exactly what will be made.'],
      ['Quick setups', 'The chips above the shapes — TikTok / Reels, YouTube 4K, YouTube 1080p60, Cinema 24, Instagram feed, Square ad, Broadcast PAL — fill the shape, size and frame rate in one press. Nothing is hidden: what a chip chose is in the controls and can still be changed. The setup you used last is the one offered next time.'],
      ['Read it off a clip', '**Match a clip…** (or drop footage anywhere on the screen) reads the clip’s frame size and frame rate — measured, so 29.97 and 23.976 come out as what they are — and sets the nearest shape, size and rate the app offers. It says what it read and what it chose, so a 4:3 clip landing in a square project is a decision you can see. Dropped footage comes in with the project: onto the timeline for a blank start or a talking head, into the bin for the builders.'],
      ['Start with', 'A blank timeline, or a door into something the editor already does: a montage built from your clips, an edit copied from a video you like, a photo dump from stills, multicam angles lined up by their sound, a talking head with the pauses cut out. Each lands you where that leads.'],
      ['Finding one', 'Search by name (press **/** from anywhere on the screen), sort by recent, name, length or most footage. A row shows the first clip’s picture, the shape, the frame rate, the footage size and when you were last in it. The arrow keys walk the list; Enter opens.'],
      ['Rename, copy, pack, delete', 'The four buttons on a row. Rename is in place — Enter keeps it, Escape does not. A copy shares the footage rather than doubling it. Pack writes one .omnidxpkg with the edit and every clip it uses, to take to another machine. Delete asks first, and can be undone for twelve seconds after; the files on your device are untouched either way.'],
      ['From another device', '**Open a project file** takes a .omnidx.json saved from another copy of the app, or a .omnidxpkg bundle. A bundle opens with its footage; a bare project file needs its media re-imported if it is not on this device — that file is the edit, not the footage.'],
    ],
  },
  {
    id: 'tabs',
