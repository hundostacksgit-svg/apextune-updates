...
-- ---------------------------------------------------------------------------
-- Support tickets: one thread between one customer and the owner.
--
-- Two readers and nobody else. The customer opens a ticket by a private link
-- (#t=<id>.<secret>); support_keys holds a hash of each secret, never the
-- secret. The owner reads on a support device: a browser that made a signing
-- key it cannot export and registered the public half here. Every owner
-- request is signed by that key and carries a time newer than the last one,
-- so a copied request cannot be sent twice. The owner token alone reads no
-- ticket: it registers the first device, and every later device waits until
-- an existing one approves its pairing code.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS support_tickets (
  id               TEXT PRIMARY KEY,      -- T-XXXXXX, what both sides quote
  topic            TEXT NOT NULL,         -- one of the page's options: no-key, key, new-pc, run, undo, before, payment, other
  email            TEXT,                  -- where the customer hears of a reply; optional
  ref              TEXT,                  -- receipt number, order email or key, as they typed it
  status           TEXT NOT NULL,         -- 'open' | 'answered' | 'closed'
  last_author      TEXT NOT NULL,         -- 'customer' | 'owner'
  created_at       INTEGER NOT NULL,
  updated_at       INTEGER NOT NULL,
  owner_seen_at    INTEGER NOT NULL DEFAULT 0,
  customer_seen_at INTEGER NOT NULL DEFAULT 0,
  owner_told_at    INTEGER NOT NULL DEFAULT 0, -- last "a ticket needs you" email: one until the owner looks
  customer_told_at INTEGER NOT NULL DEFAULT 0  -- last "support answered" email: one until the customer looks
);
CREATE INDEX IF NOT EXISTS support_tickets_updated ON support_tickets(updated_at);

CREATE TABLE IF NOT EXISTS support_messages (
  id          INTEGER PRIMARY KEY AUTOINCREMENT,
  ticket_id   TEXT NOT NULL REFERENCES support_tickets(id) ON DELETE CASCADE,
  author      TEXT NOT NULL,              -- 'customer' | 'owner'
  body        TEXT NOT NULL,
  created_at  INTEGER NOT NULL
);
CREATE INDEX IF NOT EXISTS support_messages_ticket ON support_messages(ticket_id);

-- The customer's links: the first one when the ticket opens, and one more in
-- each "support answered" email. SHA-256 of the secret, so this table opens nothing.
CREATE TABLE IF NOT EXISTS support_keys (
  hash        TEXT PRIMARY KEY,
  ticket_id   TEXT NOT NULL REFERENCES support_tickets(id) ON DELETE CASCADE,
  created_at  INTEGER NOT NULL
);
CREATE INDEX IF NOT EXISTS support_keys_ticket ON support_keys(ticket_id);

-- The owner's support devices. public_key is a P-256 public key (JWK); the
-- private half never left the browser that made it. A pending device has a
-- pairing code and reads nothing until an active one approves it.
CREATE TABLE IF NOT EXISTS support_devices (
  id           TEXT PRIMARY KEY,
  public_key   TEXT NOT NULL UNIQUE,
  label        TEXT,
  status       TEXT NOT NULL,             -- 'active' | 'pending'
  code         TEXT,                      -- pairing code while pending
  last_ts      INTEGER NOT NULL DEFAULT 0, -- newest signed request seen; an older or equal one is refused
  created_at   INTEGER NOT NULL,
  approved_at  INTEGER,
  last_seen    INTEGER
);
