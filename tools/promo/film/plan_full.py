...
def main():
    ...
    c0 = t
    dur = float(subprocess.run(['ffprobe', '-v', 'error', '-show_entries', 'format=duration', '-of', 'csv=p=0', install],
                               capture_output=True, text=True).stdout.strip() or 0) if os.path.exists(install) else 0
    i0, i1 = take.get('install', [0, dur])
    if i1 > i0:
        add(i0, i1, max(2, (i1 - i0) / 7.2), FULL, 'Clean Windows 11, from Microsoft’s own installer', 64, src='install')
    ...
    if has('setup-running', 'restarted'):
        add(V('setup-running') + 1.0, V('restarted') + 10.0, 2, FULL, 'First sign-in: OmniDx Edition sets itself up', 60)
    if has('restarted', 'welcome'):
        add(V('restarted') + 10.0, V('welcome') - 2.4, 2, FULL, 'One restart', 64)
    step('e01-welcome', 2.4, 0.6, 'And this is my Windows now', 'hub', 64)
    step('e07-desktop', 1.4, 0.4, 'Taskbar on the left, OmniDx violet')
    ...
    if has('uac-yes', 'app-read'):
        add(V('uac-yes') + 0.8, V('app-read'), 2, FULL, 'It reads the PC before it changes anything', 56)
    ...
    if has('run', 'done'):
        add(V('run') + 0.6, V('done') - 1.0, 2, FULL, 'A restore point first, then the cut', 58)
        add(V('done') - 1.0, V('done') + 1.4, 1, cam('result'), 'Done')
    if has('restart', 'three-minutes'):
        add(V('restart') - 0.5, V('three-minutes'), 2, FULL, 'Restart. Signed in. Three minutes later', 58)
