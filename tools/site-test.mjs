...
try {
  ...
  for (const p of ['', 'pricing/', 'download/', 'trust/', 'changelog/', 'what-it-touches/', 'terms/', 'activate/', 'admin/', 'account/', 'guide/', 'fresh/', 'undervolt/', 'builds/', 'edition/', 'showcase/', 'es/', 'pt/']) {
  ...
  /* 1b. Accessibility: WCAG A and AA rules from axe-core over the same pages, at phone width. Skipped, and said so, when axe is not installed. */
  {
    ...
    if (!fs.existsSync(axePath)) ok('axe-core is not installed here, so the accessibility pass is skipped (the publish check installs it)');
      ...
      const pages = ['', 'pricing/', 'download/', 'trust/', 'changelog/', 'what-it-touches/', 'terms/', 'activate/', 'admin/', 'guide/', 'fresh/', 'undervolt/', 'builds/', 'edition/', 'showcase/', 'es/', 'pt/'];
